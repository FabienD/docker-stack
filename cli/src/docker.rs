...
pub fn start(item: ComposeItem) -> Result<()> {
    execute_compose_command(item, DockerCommand::Start)
        .wrap_err("Docker start error")?;
    Ok(())    
}

pub fn stop(item: ComposeItem) -> Result<()> {
    execute_compose_command(item, DockerCommand::Stop)
        .wrap_err("Docker stop error")?;
    Ok(())    
}

pub fn restart(item: ComposeItem) -> Result<()> {
    execute_compose_command(item, DockerCommand::Restart)
        .wrap_err("Docker restart error")?;
    Ok(())    
}
